// B2 lifecycle wrappers (Step-3, sol verdict a6c75553): begin_session /
// end_session must advertise the COMMITTED wrapper contracts, so their
// entries below reference the same frozen eng4 schema objects the wrapper
// behavior is pinned to. eng4/schemas.ts is import-free — no cycle.
import {
  BEGIN_SESSION_DISCOVERY_INPUT_SCHEMA,
  CHECKPOINT_INPUT_SCHEMA,
} from '../unified-server/eng4/schemas.js';
...
export const UnifiedToolSchemas: Record<string, ToolDefinition> = {
  ...
  get_entity_detail: {
    ...
    description: 'Retrieve full entity content by storage ID, canonical entity name, or alias. `ids` preserves the scan-then-detail workflow; `names` or the singular `entity` input skips the search-then-refetch round trip and returns resolution metadata. STATE SEMANTICS: an entity\'s embedded observations[] is its creation-time definition snapshot, NOT current state — each entity result carries a resolved `currentObservation` (newest non-superseded observation); read state from that or from get_current_observation, never from the embedded array (flagged embeddedObservationsAreDefinitionSnapshot when present).',
    inputSchema: {
      type: 'object',
      properties: {
        ids: {
