...
> use the config printed by `npx -y @hythe/mcp@0.1.5 init --agent-id <agent-id>`.
...
```bash
npx -y @hythe/mcp@0.1.5 init --agent-id claude-desktop \
...
- **Name-agnostic where possible.** Directory and script names avoid the
  ...
  `mcp__hythe__*` tool names (printed by `npx -y @hythe/mcp@0.1.5 init --agent-id <agent-id>`).
