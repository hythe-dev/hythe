...
| | |
|---|---|
| Status | **ACCEPTED v5.1** — reviewed by Tomas and merged to main as 3429000 (PR #10, 2026-09-03). Implementation proceeds per §7, one gated PR per step: **H1 merged** as e678479 (PR #11, codex LGTM ee5d9f89; `heads.ts`); **H2 merged** as 16a1eee (PR #12, independent fresh-context review; `versions.ts`); **H3 merged** as 8a8da05 (PR #13, codex-hythe review 0952b074 + two independent reviews; `reconcile.ts`); **H4 merged** as b9648ea (PR #14, independent review + delta re-review + codex-hythe review; `selection.ts`); **H5 merged** as 074420c (PR #15, 2026-09-04; codex-hythe LGTM 9347d12d after four review rounds plus an independent fresh-context review; `checkpoint.ts` record/patch path and the single verified payload loader, `memo.ts`, `resource.ts` serving exactly the verified bytes, `tests/contract-eng4-h5-record-patch.test.ts`). **The H-series is complete and the v3 request/result/bundle schema is final**; publishing/deploying v3 is the owner's separate gate (§2.10) and has not happened. Implementation amendments from H3 and H5 review are recorded inline below, marked *[H3 impl]* and *[H5 impl]*. Prior draft history: v5.1 was codex-hythe's takeover revision incorporating review 12281537 while claude-hythe was temporarily disabled. §2.10 is **RULED**: the H-series is `resultVersion: 3` (Tomas, 2026-09-03). |
...
"Pointed head" (aad3973c finding 5): `expectedRevision` must equal the revision of the pointer *now*. A successful `record`/`patch` advances the pointer by §3.2a like any other write on the pointed head. In `invalid-designation`, `max-revision`, or `empty-scope` mode both ops conflict (a legacy scope must reconcile once to leave `max-revision`). *[H5 impl]* Amendment (codex review of PR #15, finding 3): in `empty-scope` mode both ops fail closed with the typed `CheckpointEmptyScopeError` instead of a `conflict` — the same fail-closed path a v3 `reconcile` on an empty scope (H3) and any non-null `expectedRevision` on an empty scope already take. A structured `conflict` there would carry an empty head set and a null pointer, which the v3 conflict schema (`heads` minItems 1) does not admit and which tells the caller nothing beyond "write first" (`expectedRevision: null`). `invalid-designation` and `max-revision` conflict as specified. *[H5 impl]* The v3 `write` fingerprint binds `acknowledgeRetired` when true (codex finding 1): the acknowledgment is admission-significant under §4.5, so a retry that drops it is an idempotency-mismatch, never a replay; `reconcile` already binds it in its normalized request.
...
`statePatch` is an RFC 7396 merge patch against the verified parent state. *[H5 impl]* The raw patch is bound into the request fingerprint; the fingerprint content carries `state: null` for record/patch (the request has none); a v3 record/patch request that carries `state` fails input validation, as does a patch without `statePatch`. Arrays replace wholesale — this does **not** solve retire-one-guardrail; that stays a separate design item. The materialized state must validate against `WORKING_STATE` or the call fails closed. `contentHash` binds the materialized envelope; `requestFingerprint` binds `operation: 'patch'` + resolved parent + the raw patch + non-state changes.
